...
type circle_visual_update_options = {
    allow_hit_animation?: boolean;
};

type circle_visual_render_options = {
...
export class CircleVisual {
    ...
    update(time: number, hit_time: number, config: DrawableConfig, options: circle_visual_update_options = {}): void {
        ...
        if (options.allow_hit_animation === false) {
            this.state.circle_alpha = 0;
            this.state.circle_scale = 1;
            this.state.number_alpha = 0;
            this.state.flash_alpha = 0;
            this.state.glow_alpha = 0;
            return;
        }

        const pre_hit_alpha = this.get_pre_hit_alpha(hit_time, appear_time, hit_time, config);
        if (has_mod(config.mods, Mods.Hidden) && pre_hit_alpha < 0.6) {
            this.state.circle_alpha = 0;
            this.state.circle_scale = 1;
            this.state.number_alpha = 0;
            this.state.flash_alpha = 0;
    ...
    render(
        ...
        combo_color: string,
        combo_count: number,
        options: circle_visual_render_options = {}
    ): void {
        const { circle_alpha, circle_scale, number_alpha, flash_alpha, glow_alpha } = this.state;

        if (circle_alpha <= 0.01 && flash_alpha <= 0.01 && glow_alpha <= 0.01) return;
        ...
        const enable_flash = options.enable_flash ?? skin.enable_hit_explode;
        const enable_glow = options.enable_glow ?? skin.enable_glow;

        if (enable_flash && flash_alpha > 0.01) {
            backend.save();
            backend.set_blend_mode("lighter");
            backend.set_alpha(flash_alpha * 0.02);
            backend.draw_circle(position[0], position[1], scaled_radius, "rgba(255,255,255,1)", "transparent", 0);
            backend.restore();
        }

        if (enable_glow && glow_alpha > 0.01) {
            backend.save();
            backend.set_blend_mode("lighter");
            backend.set_alpha(glow_alpha * skin.glow_opacity);
            const glow_color = skin.glow_use_combo_color ? combo_color : (skin.glow_color ?? combo_color);
            backend.draw_circle(position[0], position[1], scaled_radius * 1.08, glow_color, "transparent", 0);
            backend.restore();
        }
    }
    ...
    private update_post_hit(time: number, hit_time: number, config: DrawableConfig, pre_hit_alpha: number): void {
        ...
        const { skin } = config;
        const duration = Math.max(120, skin.hit_animation_duration);
        const fade_out_time = duration;
        const resize_duration = Math.max(90, duration * 0.85);
        const flash_in_duration = Math.max(30, duration * 0.25);
        const flash_out_duration = Math.max(70, duration * 0.5);
        const grow_size = skin.hit_animation_scale;

        const flash_in_progress = clamp(elapsed / flash_in_duration, 0, 1);
        const fade_out_progress = clamp(elapsed / fade_out_time, 0, 1);

        const base_alpha = has_mod(config.mods, Mods.Hidden) ? clamp(pre_hit_alpha, 0, 1) : clamp(Math.max(pre_hit_alpha, 0.25), 0, 1);
        const circle_alpha = base_alpha * (1 - Easing.OutQuad(fade_out_progress));
        const scale_progress = clamp(elapsed / resize_duration, 0, 1);
        this.state.circle_alpha = circle_alpha;
        this.state.circle_scale = lerp(1, grow_size, Easing.OutCubic(scale_progress));
        this.state.number_alpha = circle_alpha;

        if (elapsed <= flash_in_duration) {
            this.state.flash_alpha = Easing.OutQuint(flash_in_progress);
            this.state.glow_alpha = Easing.OutQuint(flash_in_progress);
        } else {
            const flash_out_progress = clamp((elapsed - flash_in_duration) / flash_out_duration, 0, 1);
            this.state.flash_alpha = 1 - Easing.OutQuint(flash_out_progress);
            this.state.glow_alpha = 1 - Easing.OutQuint(flash_out_progress);
        }
    }
